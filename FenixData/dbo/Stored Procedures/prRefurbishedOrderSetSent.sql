CREATE PROCEDURE [dbo].[prRefurbishedOrderSetSent]
(
	@RefurbishedOrderID int,
	@MessageStatusID int,	
...
-- Description  : RefurbishedOrder -> nastaveni datumu odeslani a statusu zpravy
...
---
BEGIN TRY
	UPDATE [dbo].[CommunicationMessagesRefurbishedOrder]      
	   SET [MessageDateOfShipment] = getdate()
		  ,[MessageStatusId] = @MessageStatusID
	 WHERE ID = @RefurbishedOrderID
   --
   SET @ReturnValue = 0
END TRY
BEGIN CATCH
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prRefurbishedOrderSetSent] TO [FenixW]
    AS [dbo];
