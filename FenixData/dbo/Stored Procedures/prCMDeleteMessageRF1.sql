CREATE PROCEDURE [dbo].[prCMDeleteMessageRF1]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Refurbished Confirmation (RF1)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 2. SN inactive 
		UPDATE [dbo].[CommunicationMessagesRefurbishedOrderConfirmationSerNumSent] SET IsActive=0		
		FROM [dbo].[CommunicationMessagesRefurbishedOrderConfirmationSerNumSent]  sn
		INNER JOIN [dbo].[CommunicationMessagesRefurbishedOrderConfirmationItems] ct
			ON sn.RefurbishedItemsOrKitsID = ct.ID
		INNER JOIN [dbo].[CommunicationMessagesRefurbishedOrderConfirmation]      ch
			ON ct.CMSOId = ch.ID
		WHERE ch.IsActive = 1 and ct.IsActive = 1 and sn.IsActive = 1 and ch.ID = @RF1DeleteID
		...
		-- 4.
		UPDATE [dbo].[CommunicationMessagesRefurbishedOrderConfirmation] SET [IsActive] = 0 WHERE [ID] = @RF1DeleteID AND [MessageId] = @RF1DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 5.
		UPDATE [dbo].[CommunicationMessagesRefurbishedOrderConfirmationItems] SET [IsActive] = 0 WHERE [CMSOId] = @RF1DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE RF1 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE RF1 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageRF1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 1399			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageRF1] TO [FenixW]
    AS [dbo];
