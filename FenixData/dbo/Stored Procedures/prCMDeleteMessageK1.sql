CREATE PROCEDURE [dbo].[prCMDeleteMessageK1]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Kitting Confirmation (K1)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 3.
		UPDATE [dbo].[CommunicationMessagesKittingsConfirmation] SET [IsActive] = 0 WHERE [ID] = @K1DeleteID AND [MessageId] = @K1DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 4.
		UPDATE [dbo].[CommunicationMessagesKittingsConfirmationItems] SET [IsActive] = 0 WHERE [CMSOId] = @K1DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE K1 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE K1 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageK1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 499			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageK1] TO [FenixW]
    AS [dbo];
