CREATE PROCEDURE [dbo].[prCMDeleteMessageS1]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Shipment Confirmation (S1)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 2. SN inactive 
		UPDATE [dbo].[CommunicationMessagesShipmentOrdersConfirmationSerNumSent] SET IsActive = 0
		FROM [dbo].[CommunicationMessagesShipmentOrdersConfirmationSerNumSent]  sn
		INNER JOIN [dbo].[CommunicationMessagesShipmentOrdersConfirmationItems] ct
			ON sn.ShipmentOrdersItemsOrKitsID=ct.ID
		INNER JOIN [dbo].[CommunicationMessagesShipmentOrdersConfirmation]      ch
			ON ct.CMSOId=ch.ID
		WHERE ch.IsActive = 1 and ct.IsActive = 1 and sn.IsActive = 1 and ch.ID = @S1DeleteID
		...
		-- 4.
		UPDATE [dbo].[CommunicationMessagesShipmentOrdersConfirmation] SET [IsActive] = 0 WHERE [ID] = @S1DeleteID AND [MessageId] = @S1DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 5.
		UPDATE [dbo].[CommunicationMessagesShipmentOrdersConfirmationItems] SET [IsActive] = 0 WHERE [CMSOId] = @S1DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE S1 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE S1 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageS1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 799			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageS1] TO [FenixW]
    AS [dbo];
