CREATE PROCEDURE [dbo].[prCMDeleteMessageR0]
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Created date : 2014-11-06
-- Description  : Zrušení message Reception Order (R0)
-- Returns      : 0 je-li zušení úspěšné, jinak číslo chyby
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 3.
		UPDATE [dbo].[CommunicationMessagesReceptionSent] SET [IsActive] = 0 WHERE [ID] = @R0DeleteID AND [MessageId] = @R0DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 4.
		UPDATE [dbo].[CommunicationMessagesReceptionSentItems] SET [IsActive] = 0 WHERE [CMSOId] = @R0DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE R0 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE R0 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageR0; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 199			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageR0] TO [FenixW]
    AS [dbo];
