CREATE PROCEDURE [dbo].[prCMDeleteMessageR1]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Reception Confirmation (R1)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 3.
		UPDATE [dbo].[CommunicationMessagesReceptionConfirmation] SET [IsActive] = 0 WHERE [ID] = @R1DeleteID AND [MessageId] = @R1DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 4.
		UPDATE [dbo].[CommunicationMessagesReceptionConfirmationItems] SET [IsActive] = 0 WHERE [CMSOId] = @R1DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE R1 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE R1 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageR1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 299			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageR1] TO [FenixW]
    AS [dbo];
