CREATE PROCEDURE [dbo].[prCMDeleteMessageVR3]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Returned Shipment (VR3)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 2. SN inactive 
		UPDATE [dbo].[CommunicationMessagesReturnedShipmentItemsSerNum] SET IsActive = 0
		FROM [dbo].[CommunicationMessagesReturnedShipmentItemsSerNum]  sn
		INNER JOIN [dbo].[CommunicationMessagesReturnedShipmentItems] ct
			ON sn.ReturnedShipmentItemsID = ct.ID
		INNER JOIN [dbo].[CommunicationMessagesReturnedShipment]      ch
			ON ct.CMSOId = ch.ID
		WHERE ch.IsActive = 1 and ct.IsActive = 1 and sn.IsActive = 1 and ch.ID = @VR3DeleteID
		...
		-- 4.
		UPDATE [dbo].[CommunicationMessagesReturnedShipment] SET [IsActive] = 0 WHERE [ID] = @VR3DeleteID AND [MessageId] = @VR3DeleteMessageID
		SET @myError = @myError + @@ERROR

		-- 5.
		UPDATE [dbo].[CommunicationMessagesReturnedShipmentItems] SET [IsActive] = 0 WHERE [CMSOId] = @VR3DeleteID
		SET @myError = @myError + @@ERROR

    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE VR3 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE VR3 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageVR3; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 1199			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageVR3] TO [FenixW]
    AS [dbo];
