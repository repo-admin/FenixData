...
CREATE PROCEDURE [dbo].[prCMRCconsentVR1]
			@Decision											AS Int, -- 3... zamítám, 1... schvaluji
			@Id														AS Int, -- -1... Id v tabulce [dbo].[CommunicationMessagesReturn] neodpovídá Id z přijatého XML
			                                      --  (XML message se rozpadá dle kvality) 
			@MessageId										AS Int, -- MessageID záznamu z tabulky [dbo].[CommunicationMessagesReturn]			
...
*/
	...
	END
        ...
        BEGIN TRAN
              -- 1.
						  UPDATE [dbo].[CommunicationMessagesReturn] SET [Reconciliation] = @Decision, [ModifyUserId] = @ModifyUserId WHERE [MessageID] = @MessageId
								...
								BEGIN
									...
									INSERT INTO [dbo].[CommunicationMessagesDeleteMessage]
											...
											VALUES
														...
														,@Id, @MessageId, @DeleteMessageTypeId, @DeleteMessageTypeDescription
														,NULL, NULL, NULL
...
BEGIN CATCH
      ...
      SET @sub = 'FENIX - Returned Equipment CHYBA' + ' Databáze: '+ISNULL(@myDatabaseName,'')
      SET @msg = 'Program prCMRCconsentVR1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMRCconsentVR1] TO [FenixW]
    AS [dbo];
