CREATE PROCEDURE [dbo].[prCMDeleteMessageVR1]                       
(
		@ID int, 
		@MessageID int, 
		@MessageTypeID int, 
...
-- Description  : Zrušení message Returned Equipment (VR1)
...
BEGIN TRY
    ...
    SET @myAdresaProgramator = [dbo].[fnHomeAdresses] (2)
		...
		-- 2. SN inactive
		UPDATE [dbo].[CommunicationMessagesReturnItemsSN] SET IsActive = 0	
		FROM [dbo].[CommunicationMessagesReturnItemsSN]    Sn
		INNER JOIN [dbo].[CommunicationMessagesReturn]     H
			ON Sn.CMSOId = H.ID
		WHERE Sn.IsActive = 1 AND H.IsActive = 1 AND H.[MessageId] = @VR1DeleteMessageID
		
		-- 3. Item inactive
		UPDATE [dbo].[CommunicationMessagesReturnItems] SET IsActive = 0
		FROM [dbo].[CommunicationMessagesReturnItems]				I
		INNER JOIN [dbo].[CommunicationMessagesReturn]			H
			ON   I.CMSOId = H.ID
		WHERE I.IsActive = 1 AND H.IsActive = 1 AND H.[MessageId] = @VR1DeleteMessageID

		-- 4. Header inactive
		UPDATE [dbo].[CommunicationMessagesReturn] SET [IsActive] = 0 WHERE [MessageId] = @VR1DeleteMessageID
		SET @myError = @myError + @@ERROR
    ...
    IF @myError = 0 
			BEGIN 
					COMMIT TRAN
					SET @sub = 'FENIX - DELETE MESSAGE VR1 - Oznámení' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
		...
		ELSE
			BEGIN 
					...
					SET @sub = 'FENIX - DELETE MESSAGE VR1 - PROBLÉM !!!' + ' Databáze: ' + ISNULL(@myDatabaseName,'')
...
BEGIN CATCH
      ...
      SET @msg = 'Program prCMDeleteMessageVR1; '  + ISNULL(ERROR_MESSAGE(),'') + ISNULL(CAST(ERROR_NUMBER() AS VARCHAR(50)),'') 
      EXEC  @result = msdb.dbo.sp_send_dbmail
			...
			--SELECT 999			-- TODO zaremovat
END CATCH
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[prCMDeleteMessageVR1] TO [FenixW]
    AS [dbo];
